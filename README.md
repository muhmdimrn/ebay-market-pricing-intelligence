...
2. **Free shipping is now the market norm**, not the exception. Roughly 75-77% of listings offer it, meaning sellers who charge separately for shipping may appear less competitive by comparison.
...
Below are the key DAX measures used to power the dashboard's KPI cards, beyond simple column aggregations (average, count).

**Free Shipping Rate**
```dax
% Free Shipping =
DIVIDE(
    CALCULATE(COUNTROWS('public ebay_listings'), 'public ebay_listings'[shipping_cost] = 0),
    COUNTROWS('public ebay_listings')
)
```
Calculates the share of listings with zero shipping cost, used to measure how common free shipping is across the market.

**Day-over-Day Price Growth**
```dax
Price Growth =
VAR CurrentAvg = CALCULATE(AVERAGE('public ebay_listings'[price]), 'public ebay_listings'[snapshot_date] = MAX('public ebay_listings'[snapshot_date]))
VAR PreviousAvg = CALCULATE(AVERAGE('public ebay_listings'[price]), 'public ebay_listings'[snapshot_date] = MAX('public ebay_listings'[snapshot_date]) - 1)
RETURN
DIVIDE(CurrentAvg - PreviousAvg, PreviousAvg)
```
Compares today's average price against the previous day's, expressed as a percentage change. Relies on `VAR` to avoid repeating the same filter context twice, then returns the percentage difference between the two.

**New Condition Rate**
```dax
% New Condition =
DIVIDE(
    CALCULATE(COUNTROWS('public ebay_listings'), 'public ebay_listings'[condition] = "New"),
    COUNTROWS('public ebay_listings')
)
```
Same pattern as Free Shipping Rate, applied to the condition field instead.

**Top Category (by average price)**
```dax
Top Category =
CALCULATE(
    VALUES('public ebay_listings'[category]),
    TOPN(1, VALUES('public ebay_listings'[category]), CALCULATE(AVERAGE('public ebay_listings'[price])), DESC)
)
```
Returns the name of whichever category currently has the highest average price. `TOPN` ranks each category by its average price and keeps only the top result, so this measure updates automatically as pricing shifts day to day.

**Top Location (by listing count)**
```dax
Top Location =
CALCULATE(
    VALUES('public ebay_listings'[seller_location]),
    TOPN(1, VALUES('public ebay_listings'[seller_location]), CALCULATE(COUNTROWS('public ebay_listings')), DESC)
)

Top Location Share =
DIVIDE(
    CALCULATE(COUNTROWS('public ebay_listings'),
        TOPN(1, VALUES('public ebay_listings'[seller_location]), CALCULATE(COUNTROWS('public ebay_listings')), DESC)),
    COUNTROWS('public ebay_listings')
)
```
The same `TOPN` ranking pattern used for Top Category, applied to seller location instead, paired with a second measure that calculates what share of all listings come from that top location.

**Price Spread**
```dax
Price Spread = MAX('public ebay_listings'[price]) - MIN('public ebay_listings'[price])
```
The simplest measure here, just the gap between the highest and lowest priced listing currently tracked.

**High Price Bucket Rate**
```dax
% High Price Bucket =
DIVIDE(
    CALCULATE(COUNTROWS('public ebay_listings'), 'public ebay_listings'[price_bucket] = "High"),
    COUNTROWS('public ebay_listings')
)
```
Same ratio pattern as the other percentage measures, applied to the pre-computed `price_bucket` column (Low/Mid/High, calculated during the Python transform step).
